...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("CryptoLottoToken version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  CryptoLottoTokend [options]                     " + "\n" +
                  "  CryptoLottoTokend [options] <command> [params]  " + _("Send command to -server or CryptoLottoTokend") + "\n" +
                  "  CryptoLottoTokend [options] help                " + _("List commands") + "\n" +
                  "  CryptoLottoTokend [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "CryptoLottoToken:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: CryptoLottoToken.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: CryptoLottoTokend.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 9157 or testnet: 19157)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9154 or testnet: 19154)") + "\n" +
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CryptoLottoToken is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "CryptoLottoToken server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CryptoLottoToken") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart CryptoLottoToken to complete") << "\n";
