...
// Copyright (c) 2011-2012 CryptoLottoToken Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("CryptoLottoToken-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CryptoLottoTokenTEST0\r");
            Send(hSocket, "WHO #CryptoLottoTokenTEST0\r");
        } else {
            // randomly join #CryptoLottoToken00-#CryptoLottoToken99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #CryptoLottoToken%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CryptoLottoToken%02d\r", channel_number).c_str());
        }
