...
uint256 hashGenesisBlock("0x00000326fcb9e2d71672332c1cf1e525c08323d247cad5b16b3bbf71d497aa01");
...
const string strMessageMagic = "CryptoLottoToken Signed Message:\n";
...
{
    // CryptoLottoToken: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // CryptoLottoToken: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CLT Genesis phrase";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045d4a201dd76decf67f2338ce5ecb29ea0429f339a7a2e2d5125851a4849dccbf64a58d1b469c4ce4a26b4806905c936988a075f0a93af946b99b657c1202d2fc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1474659365;
       ...
       if (false && (block.GetHash() != hashGenesisBlock)) {
        ...
        assert(block.hashMerkleRoot == uint256("0x32832a2dfbffa0f71e16397f10c38569bd6eb2af3f7f860f0b8012bc9473ff38"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc1, 0xa7, 0xee, 0xc2 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptoLottoTokenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoLottoTokenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptoLottoTokenMiner(CWallet *pwallet)
{
    printf("CryptoLottoTokenMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CryptoLottoToken-miner");
    ...
    {
        printf("CryptoLottoTokenMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptoLottoTokenMiner, pwallet));
}
